...
Це початковий архітектурний дизайн. Для v0.1 фактичні модулі та API уточнені нижче; критерії продуктивності є цілями приймання.

## Фактичні модулі v0.1

- packages/core: симуляція та боти; createWorld(seed, config), step(world, commands), snapshot(world).
- packages/protocol: схеми та валідація повідомлень.
- apps/client/main.ts: локальна сесія, меню й цикл кадру; network.ts: мережева сесія.
- apps/client/input.ts і xr.ts: desktop та VR; render.ts: графіка.
- apps/server: авторитетний світ, кімнати, HTTP/WSS.

Окремі packages/bots та абстрактний Session не створювалися: у поточному обсязі вони не додають корисної ізоляції. Сигнатури в розділі початкових контрактів нижче є ескізом, точні типи знаходяться у вихідному коді.
